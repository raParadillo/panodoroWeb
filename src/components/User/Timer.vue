<template>
    ...
    <div class="timer-card">
        ...
        <button 
            ...
            @click="setMode('pomodoro', 25)"
        >Panodoro</button>
        <button 
            :class="{ active: currentMode === 'longBreak' }" 
            @click="setMode('longBreak', 15)"
        >Long break</button>
        <button 
            :class="{ active: currentMode === 'shortBreak' }" 
            @click="setMode('shortBreak', 5)"
        >Short break</button>
        </div>
        ...
        <button class="icon-btn play-pause" @click="toggleTimer" title="Play/Pause">
            <span v-if="!isRunning">
            <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="currentColor" width="32" height="32"><path d="M8 5v14l11-7z"/></svg>
            </span>
            <span v-else>
            <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="currentColor" width="32" height="32"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>
            </span>
        </button>
        </div>
    </div>
...
.tabs button.active {
    background: #6d4c41; /* Brown tone matched to your active selection highlight */
